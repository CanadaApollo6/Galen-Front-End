...
const getDataSets = (
    ...
    const mapTargetToDataSet = (target: RnDeltaType, well: string) => ({
        ...
        data: rns[well][target],
        borderColor: config[target]["color"],
        fill: false,
    });
        ...
        .map((well) =>
            target
                ...
                : Object.keys(config).map((k) =>
                      mapTargetToDataSet(k as RnDeltaType, well)
                  )
        )
        .flat();
};
...
export type SampleChartProps = {
    ...
    target: RnDeltaType | undefined | "";
    wells: string[];
    graphType: "linear" | "logarithmic";
};

export const SampleChart: React.FC<SampleChartProps> = ({
    scale,
    target,
    graphType,
    wells,
