...
const ImportQuantFileButton: React.FC = () => {
    ...
    const onFileSelect = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
        ...
        setDeterminations(samples.map(({ sample_id, determination, prediction, confidence, amplifications, well, evaluated, elution }) =>
            ({ sample_id, determination, prediction, confidence, amplifications, well, evaluated, elution })))
    ...
    return (
        <Button type='primary' disabled={file && determinations.length === 0}>
            <label>
                <input ref={input} id='quant-file' type='file' accept='.txt' style={{ display: 'none' }} onChange={onFileSelect} />
