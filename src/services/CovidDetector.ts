...
export default async ({ rns: { n_gene_delta, orf1ab_delta, s_gene_delta } }: ExportSample): Promise<[Determination, number]> => {
    const normalized = [normalize(n_gene_delta), normalize(orf1ab_delta), normalize(s_gene_delta)]
    ...
    if (prediction) {
        ...
        if (Array.isArray(array)) {
            ...
            return [ai_value < .5 ? 'Detected' : 'Not Detected', confidence]
        }
    }
