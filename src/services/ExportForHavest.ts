...
const determinationToCovid = ({ determination, sample_id }: SampleDetermination) =>
...
const determinationToSARS2TP = ({ determination, sample_id }: SampleDetermination) =>
    `"${sample_id}",SARS2TP,"${determinationToSARS2TPResult(determination)}"`

export const parseCsvContent = (determinations: SampleDetermination[]): string => {
    ...
    return header.concat(covid, covcol, sars2tp).reduce((csv, row) => csv + row + '\n', '')
}
