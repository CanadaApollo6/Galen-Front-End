...
export default async ({
    rns: { rp_cy5_delta, n_gene_delta, s_gene_delta, orf1ab_delta, ms2_delta },
}: ExportSample): Promise<[Determination, number]> => {
    const model = await tf.loadLayersModel('vijs/model.json')
    const normalized = [
        rp_cy5_delta,
        n_gene_delta,
        s_gene_delta,
    ...
    if (prediction) {
        ...
        if (Array.isArray(array)) {
            ...
            return [ai_value < 0.5 ? 'Not Detected' : 'Repeat', confidence]
        }
    }
