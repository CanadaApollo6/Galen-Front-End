...
const resultMapper: QuantMapper<ResultRecord> = (data) => ({
    well: parseInt(data[0]),
    well_position: data[1],
    sample_id: data[2],
    target: data[3],
});
...
const parseQuantExport = async (file: File): Promise<QuantExport> => {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i] === "[Amplification Data]") {
            const [amplification, new_index] = pullDataSet(
                amplificationMapper,
                lines,
                i + 2
            );
        ...
        if (lines[i] === "[Results]") {
            const [results, new_index] = pullDataSet(
                resultMapper,
                lines,
                i + 2
            );
...
const getElutionPlate = (i: number) =>
    parseInt((i % 48 > 23 ? "1" : "0") + (i % 2 ? "1" : "0"), 2) + 1;
...
export default async (file: File): Promise<ExportSample[]> => {
    ...
    for (let amplification of quant_export.amplification) {
        const target = amplification.target
            .replace(" ", "_")
            .replace("-", "_")
            .toLowerCase();
    ...
    const samples = quant_export.results
        .filter((q) => q.target === "MS2")
        .map(
            (q, i): ExportSample => ({
